...
class CharacterList extends Component {
  ...
  onLoadMore = offset => {
    ...
    this.marvelServer.getAllCharacters(offset).then(this.onCharAllLoaded).catch(this.onError);
  };
  ...
  render() {
    ...
    const elementsList = charList.map(value => {
      return (
        <div key={value.id} className="character_list_item" onClick={() => this.props.onCharSelected(value.id)} tabIndex="0" onKeyDown={e => this.onEnterPush(e, value.id)}>
